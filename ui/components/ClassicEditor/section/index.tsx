...
export const Section: FC<any> = ({ instruction, content, initExpanded = false, deleteItem }) => {
  ...
  return (
    <section className={`flex p-5 m-5 border border-border bg-backg rounded-md relative`}>
      ...
      <div className={` flex w-full mini-scrollbar transition-all duration-500 overflow-x-auto overflow-y-hidden ${expanded ? "h-64" : " overflow-x-hidden h-0"} ${hidden ? "hidden" : "block"}`}>
        {
