...
const IDL_DEFAULT_PATH: &str = "/Users/juan/soda/idl.json";
...
fn greet(name: &str) -> String {
          ...
          Ok(val) => {
              ...
              if path.split('.').count() > 1 {
                  let helper_name = path
                      ...
                      .last()
                      .unwrap();
                  handlebars.register_script_helper_file(helper_name, (*path).to_string()).unwrap();
              }
          }
          Err(err) => println!("{}", err),
      }
  ...
  for entry in WalkDir::new(format!("{}/files/", template_path)) {
      ...
      if path.split('.').last().unwrap() == "hbs" {
          ...
          handlebars.render_to_write("template", &idl, &mut output_lib_file).unwrap();
          println!("{}", file_path);
      } else {
